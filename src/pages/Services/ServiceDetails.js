...
const ServiceDetails = () => {
  ...
  console.log(service);
  const { name, img, price, desc, rating, userName, userPhoto, review } = service;
  return (
	...
	<div className="flex justify-between p-4">
		<div className="flex space-x-4">
			<div>
				<img src={userPhoto} alt="" className="object-cover w-12 h-12 rounded-full dark:dark:bg-gray-500" />
			</div>
			<div>
...
<p className=" text-lg text-center">Please login to add a review. <Link to='/login'><span className="text-yellow-500 underline">Login now</span></Link></p>
          </div>
        </div>
      </div>
    </div>
